...
public class BarChartM extends JFrame{
    LinkedList<Statistics> stats;
    
    public BarChartM(String appTitle, String chartTitle, 
            LinkedList<Statistics> stats){
        super(appTitle);
        this.stats = stats;
        ...
        try{
            ChartUtilities.saveChartAsPNG(new File("charts/mchart.png"), chart,
                    600, 500);
        }catch(Exception e){
            e.printStackTrace();
        }
    ...
    private CategoryDataset createDataset(){
        ...
        while(statIt.hasNext()){
            inStat = statIt.next();
            dataset.addValue(inStat.getM(), "μ",
        ...
        private JFreeChart createChart(CategoryDataset dataset, String chartTitle){
            JFreeChart chart = ChartFactory.createBarChart3D(chartTitle, "Μέσος "
                    + "ρυθμός στη φάση της εξυπηρέτησης (μ)",
